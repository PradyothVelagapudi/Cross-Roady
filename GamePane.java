...
public class GamePane extends BorderPane {
	...
	public GamePane() {
		...
		alive = true;
		numLives = 5;
		//Create a new Animation Timer and start it
		t = new Timer();
		t.start();
		
		//Build obstacle objects out of JavaFX Shapes
		road1 = new Rectangle();
		road1.setX(0);
		road1.setY(600);
		road1.setWidth(800);
		...
		//Build chicken (player character)
		...
		//Create animations for each moving element
		...
		//Create death message and background box. These will be updated to become visible only upon death.
		...
		//Show the remaining lives in the top right corner
	...
	public class Timer extends AnimationTimer {
		public void handle(long now) {
			//Chicken movement: if the boolean value corresponding to a certain direction is true, then
			//move the chicken 3 pixels in that direction. However, the chicken is not allowed to move 
			//if doing so would take it out of window bounds.
			...
			//Collision check: If the chicken is touching an obstacle, then it dies
			...
			if (chicken.getBoundsInParent().intersects(car1.getBoundsInParent())
				...
				if (numLives > 1) { //if the chicken still has more lives remaining: the player is prompted to restart
					numLives -= 1;
					livesMessage.setText("Number of Lives: " + numLives);
					deathMessage.setText("You died!");
					restartMessage.setText("Press 'R' to restart.\n" + numLives + " lives remaining.");
				} else { //if the chicken has 0 lives remaining: this means the player has lost.
					numLives -= 1;
					livesMessage.setText("Number of Lives: " + numLives);
			...
			//Collision check: victory check. If the chicken is touching the beehive, the player wins
	...
	private class KeyHandler implements EventHandler<KeyEvent> {
		...
		public void handle(KeyEvent event) {
			//Movement: Pressing WASD updates the boolean values wPressed, sPressed, aPressed, dPressed which will
			//execute chicken movement in the Animation Timer.
			...
			//Restart: This input option is only available in the death screen, and takes the player 
			//back to the starting point while restarting the paused animation.
